...
exports.socketio = (hookName, args, cb) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('getPersona', async (data, respond) => {
      ...
      // other's updates.
      socket.join(padId);
      respond(await commentManager.getPersona(padId));
    });
    ...
    socket.on('bulkAddComment', async (padId, data, respond) => {
      ...
      respond(_.object(commentIds, comments)); // {c-12p:data, c-124:data}
    });
    ...
    socket.on('updatePersona', async (data, respond) => {
      try {
        // const {commentId, commentText, authorId} = data;
        const {persona,padId} = data;
        // const {padId} = await readOnlyManager.getIds(data.padId);
        //await commentManager.changeCommentText(padId, commentId, commentText, authorId);
        await commentManager.changePersona(padId, persona);
        //socket.broadcast.to(padId).emit('textCommentUpdated', commentId, commentText);
        respond('');
      } catch (err) {
        respond(err.message || err.toString());
...
exports.padInitToolbar = (hook, args, cb) => {
  const toolbar = args.toolbar;
  const apis = ['sentence analysis','generate headline','paraphrase', 'spellcheck'];
  const persona = ["Innocent", "Everyman", "Hero", "Outlaw", "Explorer", "Creator", "Ruler", "Magician", "Lover", "Caregiver", "Jester", "Sage"]
  const preview = "preview";

  const previews = toolbar.selectButton({
    command: 'preview',
    class: 'preview-selection',
    selectId: 'preview-selection',
  })
  previews.addOption('preview','previews',{'data-l10n-id': 'ep_comments_page.preview'})
  const personas = toolbar.selectButton({
    command: 'personas',
    class: 'persona-selection',
    selectId: 'persona-selection',
  });
  personas.addOption('dummy', 'personas', {'data-l10n-id': 'ep_comments_page.personas'});
  toolbar.registerButton('preview', previews);
  
  persona.forEach((per, value) => {
    personas.addOption(value, per, {'data-l10n-id': `ep_comments_page.${per}`});
  });
  toolbar.registerButton('persona', personas);

  const mlapi = toolbar.selectButton({
    command: 'mlapi',
    class: 'api-selection',
    selectId: 'api-selection',
  });
  mlapi.addOption('dummy', 'mlapis', {'data-l10n-id': 'ep_comments_page.mlapis'});
  apis.forEach((api, value) => {
    mlapi.addOption(value, api, {'data-l10n-id': `ep_comments_page.${api}`});
  });

  toolbar.registerButton('mlapi', mlapi);
  return cb();
};
...
exports.eejsBlock_editbarMenuLeft = (hook, args, cb) => {
  if (JSON.stringify(settings.toolbar).indexOf('mlapi') > -1) {
    return cb();
  }
  args.content += eejs.require('ep_comments_page/templates/editbarButtons.ejs');
  return cb();
};
...
exports.eejsBlock_scripts = (hookName, args, cb) => {
  ...
  args.content += eejs.require('ep_comments_page/templates/popups.html');
  return cb();
};
